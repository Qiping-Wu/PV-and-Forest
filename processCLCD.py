...
def count_pixels(tif_path):
    ...
    pixels_value_2 = np.sum(data == 2)
    print(f"forest number of {tif_path}：{pixels_value_2}")
    dataset = None
    
def process_CLCD(input_dir,outpath,clip_pv):
    tif_files=os.listdir(input_dir)
    for filename in tif_files:
        if os.path.splitext(filename)[1]=='.tif':
            year=os.path.splitext(filename)[0][-11:-7]
            CLCDds=gdal.Open(input_dir+'/'+filename,gdal.GA_ReadOnly)
            proj=CLCDds.GetProjection()
            gt=CLCDds.GetGeoTransform()
            CLCDArr=CLCDds.ReadAsArray()
            
            forest_values=[2]
            nodata_value=0
            forestArr=np.full_like(CLCDArr,nodata_value,dtype=CLCDArr.dtype)
            mask=np.isin(CLCDArr,forest_values)
            forestArr[mask]=CLCDArr[mask]
            
            forest_outpath=outpath+'CLCDForest'+year+'.tif'
            array_to_raster(forest_outpath, forestArr, proj, gt, driver_name='GTiff', nodata=0)
            
            clip_outpath=outpath+'CLCDClip'+year+'.tif'
...
def main(argv):
    ...
    process_CLCD(input_dir,outpath,clip_pv)
