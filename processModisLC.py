...
def main(argv):
    input_dir='/dat1/PV and forest/MODIS'
    outpath='/dat1/qiping_data/PV And Forest/WSP_LULC/ModisLC/'
    ...
    # Land cover type 1 
    tif_files=list()
    tif_path_list=os.listdir(input_dir)
    for filename in tif_path_list:
        if os.path.splitext(filename)[1]=='.tif':
            tif_files.append(input_dir+'/'+filename)
    
            
    merge_options=gdal.WarpOptions(format='GTiff',resampleAlg='near',srcNodata=None, dstNodata=0,creationOptions=["COMPRESS=LZW"])
    merge_outpath=outpath+'ModisLC_Mrg.tif'
    gdal.Warp(merge_outpath, tif_files,options=merge_options)
    
    
    resample_outpath=outpath+'ModisLC_Resample.tif'
    path_refer='/dat1/qiping_data/PV And Forest/WSP_forest/output/forestation_buf2km.tif'
    resample_images(path_refer, merge_outpath, resample_outpath,nodata=0)
    
    
    clip1_outpath = outpath+"ModisLC_buf2km.tif"
    clip2_outpath = outpath+"ModisLC_CN.tif"
