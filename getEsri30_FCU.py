...
def main(argv):
    ...
    #  Esri30
    tif_Esri30=input_dir+'Esri10/Esri30_buf2km.tif'
    Esri30_ds=gdal.Open(tif_Esri30,gdal.GA_ReadOnly)
    geotransform=Esri30_ds.GetGeoTransform()
    projection=Esri30_ds.GetProjection()
    Esri30_arr=Esri30_ds.ReadAsArray()
    Esri30_ds=None
    
    # forest: 2
    value_forest=[2]
    Esri30_forest_arr=np.where(np.isin(Esri30_arr,value_forest),1,0) 
    Esri30_forest_outpath=outpath+'Esri30_forest.tif'
    array_to_raster(Esri30_forest_outpath, Esri30_forest_arr, projection, geotransform, driver_name='GTiff', nodata=0)
    
    # cropland: 5
    value_cropland=[5]
    Esri30_cropland_arr=np.where(np.isin(Esri30_arr,value_cropland),1,0) 
    Esri30_cropland_outpath=outpath+'Esri30_cropland.tif'
    array_to_raster(Esri30_cropland_outpath, Esri30_cropland_arr, projection, geotransform, driver_name='GTiff', nodata=0)
    
    # urban: 7
    value_urban=[7]
    Esri30_urban_arr=np.where(np.isin(Esri30_arr,value_urban),1,0) 
    Esri30_urban_outpath=outpath+'Esri30_urban.tif'
    array_to_raster(Esri30_urban_outpath, Esri30_urban_arr, projection, geotransform, driver_name='GTiff', nodata=0)
