...
def main(argv):
    ...
    #  ESACCI 
    tif_ESACCI=input_dir+'ESA_CCI/ESACCI_buf2km.tif'
    ESACCI_ds=gdal.Open(tif_ESACCI,gdal.GA_ReadOnly)
    geotransform=ESACCI_ds.GetGeoTransform()
    projection=ESACCI_ds.GetProjection()
    ESACCI_arr=ESACCI_ds.ReadAsArray()
    ESACCI_ds=None
    
    # forest: 50,60,61,62,70,71,72,80,81,82,90,100,160,170
    value_forest=[50,60,61,62,70,71,72,80,81,82,90,100,160,170]
    ESACCI_forest_arr=np.where(np.isin(ESACCI_arr,value_forest),1,0) 
    ESACCI_forest_outpath=outpath+'ESACCI_forest.tif'
    array_to_raster(ESACCI_forest_outpath, ESACCI_forest_arr, projection, geotransform, driver_name='GTiff', nodata=0)
    
    # cropland: 10,11,12,20,30,40
    value_cropland=[10,11,12,20,30,40]
    ESACCI_cropland_arr=np.where(np.isin(ESACCI_arr,value_cropland),1,0) 
    ESACCI_cropland_outpath=outpath+'ESACCI_cropland.tif'
    array_to_raster(ESACCI_cropland_outpath, ESACCI_cropland_arr, projection, geotransform, driver_name='GTiff', nodata=0)
    
    # urban: 190
    value_urban=[190]
    ESACCI_urban_arr=np.where(np.isin(ESACCI_arr,value_urban),1,0) 
    ESACCI_urban_outpath=outpath+'ESACCI_urban.tif'
    array_to_raster(ESACCI_urban_outpath, ESACCI_urban_arr, projection, geotransform, driver_name='GTiff', nodata=0)
