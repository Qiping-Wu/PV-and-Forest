...
def count_pixels(tif_path):
    ...
    pixels_value_1 = np.sum(data == 1)
    pixels_value_2 = np.sum(data == 2)
    pixels_value_3 = np.sum(data == 3)
    print(f"high-confidence forest number：{pixels_value_3}")
    print(f"medium-confidence forest number：{pixels_value_2}")
    print(f"low-confidence forest number：{pixels_value_1}")
    dataset = None
...
def main(argv):
    ...
    prj_outpath=outpath+'forestation_prj.tif'
    Prj=gdal.Warp(prj_outpath,forestation_outpath, options=proj_ops)
    Prj=None

    clip_outpath = outpath+"/clip_forestation.tif"
    clip(clip_pv, prj_outpath, clip_outpath)
